...
// GET /api/excel/template-employees — employee template only
router.get('/template-employees', adminAuth, (req, res) => {
...
// GET /api/excel/template-installations — daily installations template
router.get('/template-installations', adminAuth, (req, res) => {
  const wb = XLSX.utils.book_new();
  ...
  const instData = [
    ['Employee ID', 'Name', 'Installations', 'Date'],
    ['1234567', 'Arjun Sharma', 5, todayStr],
    ['7654321', 'Priya Mehta', 3, todayStr],
    ['9876543', 'Ravi Kumar', 7, todayStr],
  ];
  const instSheet = XLSX.utils.aoa_to_sheet(instData);
  instSheet['!cols'] = [{ wch:14 },{ wch:24 },{ wch:15 },{ wch:14 }];
  XLSX.utils.book_append_sheet(wb, instSheet, 'Installations');
  const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
  res.setHeader('Content-Disposition', 'attachment; filename="installations-template.xlsx"');
  res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  res.send(buf);
});

// GET /api/excel/template — keep old combined template for backward compat
router.get('/template', adminAuth, (req, res) => {
  ...
  const instData = [['Employee ID', 'Name', 'Installations', 'Date'], ['1234567', 'Arjun Sharma', 5, todayStr]];
  const instSheet = XLSX.utils.aoa_to_sheet(instData);
  XLSX.utils.book_append_sheet(wb, instSheet, 'Installations');
  const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
...
// POST /api/excel/import-employees — ONLY employees, no installations
router.post('/import-employees', adminAuth, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    saveDB();
    res.json({ success: true, empAdded, empUpdated, newPasswords });
  } catch (e) {
    res.status(400).json({ error: 'Import failed: ' + e.message });
  }
});

// POST /api/excel/import-installations — ONLY installations, fast
router.post('/import-installations', adminAuth, upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
  try {
    const wb = XLSX.read(req.file.buffer, { type: 'buffer' });
    const installations = parseInstallations(wb);
    const db = getDB();
    ...
    try {
      for (const inst of installations) {
        ...
        if (!empCheck.length || !empCheck[0].values.length) {
          skipped.push(inst.employee_id);
          continue;
        }
        db.run(
    ...
    saveDB();
    res.json({ success: true, instUpdated, skipped: skipped.length, total: installations.length });
  } catch (e) {
    res.status(400).json({ error: 'Import failed: ' + e.message });
  }
});

// POST /api/excel/preview-employees
router.post('/preview-employees', adminAuth, upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
  try {
    const wb = XLSX.read(req.file.buffer, { type: 'buffer' });
    const employees = parseEmployees(wb);
    res.json({ employees: employees.slice(0, 3), total: employees.length, preview: true });
  } catch (e) {
    res.status(400).json({ error: 'Invalid file: ' + e.message });
  }
});

// POST /api/excel/preview-installations
router.post('/preview-installations', adminAuth, upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
  try {
    const wb = XLSX.read(req.file.buffer, { type: 'buffer' });
    const installations = parseInstallations(wb);
    res.json({ installations: installations.slice(0, 3), total: installations.length, preview: true });
  } catch (e) {
    res.status(400).json({ error: 'Invalid file: ' + e.message });
  }
});

// POST /api/excel/preview (old combined)
router.post('/preview', adminAuth, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const employees = parseEmployees(wb);
    const installations = parseInstallations(wb);
    res.json({ employees, installations, preview: true });
  } catch (e) {
    res.status(400).json({ error: 'Invalid Excel file: ' + e.message });
  }
});

// POST /api/excel/import (old combined — keep for backward compat)
  ...
  try {
    ...
    try {
      for (const emp of employees) {
        ...
        if (!exists.length || !exists[0].values.length) {
          const hash = bcrypt.hashSync(pwd, BULK_ROUNDS);
          db.run(`INSERT INTO employees (employee_id, name, category, country, region, state, mobile, password_hash, active, must_change_password) VALUES (?,?,?,?,?,?,?,?,?,0)`,
            [emp.employee_id, emp.name, emp.category, emp.country || 'India', emp.region, emp.state, mobile, hash, emp.active ? 1 : 0]);
          newPasswords.push({ employee_id: emp.employee_id, name: emp.name, mobile: mobile ? mobile.slice(0,5)+'*****' : '—', password: pwd });
          empAdded++;
        } else {
          db.run(`UPDATE employees SET name=?, category=?, country=?, region=?, state=?, active=? WHERE employee_id=?`,
            [emp.name, emp.category, emp.country || 'India', emp.region, emp.state, emp.active ? 1 : 0, emp.employee_id]);
          if (mobile) { const hash = bcrypt.hashSync(pwd, BULK_ROUNDS); db.run("UPDATE employees SET mobile=?, password_hash=? WHERE employee_id=?", [mobile, hash, emp.employee_id]); }
          empUpdated++;
        }
      }
      for (const inst of installations) {
        ...
        db.run(`INSERT INTO points (employee_id, points, week, month, year) VALUES (?,?,?,?,?) ON CONFLICT(employee_id, week, year) DO UPDATE SET points = points + excluded.points, updated_at = CURRENT_TIMESTAMP`,
          [inst.employee_id, inst.installations, inst.week_of_month, inst.month, inst.year]);
        instUpdated++;
      }
      db.run('COMMIT');
    ...
    saveDB();
    res.json({ success: true, empAdded, empUpdated, instUpdated, newPasswords });
  } catch (e) {
    res.status(400).json({ error: 'Import failed: ' + e.message });
  }
});
...
function parseEmployees(wb) {
  const employees = [];
  if (!wb.SheetNames.includes('Employees')) return employees;
...
function parseInstallations(wb) {
  ...
  rows.forEach((row) => {
    ...
    if (!week_of_month) {
      ...
      week_of_month = dayToWeekOfMonth(new Date().getDate());
    }
